...
| 35 | SNMP throughput against this plane tops out at roughly **6 polls/second in total**, regardless of concurrency: 40 endpoints take 9.1 s at 8-way and 7.0 s at 40-way - five times the parallelism for 1.31x the throughput | a full sweep of 598 pollable devices projects to ~1m45s against a 30 s interval. The limit is snmpsim serving the whole fleet from one process behind a wildcard socket, not the collector, and no amount of `max_concurrent` will move it. Real gear answers per device and does not share the ceiling - but any capacity conclusion drawn from THIS plane is a conclusion about snmpsim |
