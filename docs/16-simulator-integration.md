...
| 31 | **CORRECTED.** SNMP and gNMI do NOT agree on interface identity by default. gNMI reports `GigabitEthernet0/43`; SNMP reports `43`, because a table's instance is its row's OID suffix (ifIndex) unless the mapping sets `instance_from` - and `index_oid` alone does not do that | one port became two series with ZERO overlap across 65 ports. It stayed invisible while gotcha 32 kept ifXTable unreadable, and a phase-3.7 check that appeared to show agreement was a bad SQL join counting each gNMI sample once per protocol endpoint. Fixed by keying ifXTable on the ifName value; ifTable has no ifName column so its rows stay ifIndex-keyed and the ingest worker resolves them against inventory |
| 32 | **FIXED 2026-08-19.** The SNMP dataset wrote some ASN.1 type tags in HEX-as-decimal: `ifHCInOctets`/`ifHCOutOctets` used `44` and the ifTable error/discard columns `41`, where .snmprec wants the decimal tag (**70** Counter64, **65** Counter32). snmpsim builds its table from pysnmp's tag sets and RAISES on anything else, so those records took out the request that landed on them | SNMP served no interface traffic at all, and the whole ifXTable died on any device whose walk reached `ifHCInOctets`. Fixed in `core/snmprec_generator.py` and `core/mac_table_generator.py` (36 tags) with the type table in the module docstring corrected - it had documented `41 = Counter32` and `65 = NetworkAddress`, which is what caused it |
| 33 | SNMP poll latency is dominated by CONTENDING LOAD, not by the plane. The same 65-port switch polls in **1.1 s** with nothing else running and **90 s** while a 1386-endpoint collector is polling; an early 5.5 s average was measured at load average 19 | measure with the collector stopped, or the number describes the measurement. This bit twice: once diagnosing SNMP capacity, and again reading a post-fix timeout as a regression when it was contention |
| 35 | SNMP throughput is capped by the responder, not by concurrency, and the cap tightened once the type-tag fix doubled what it serves. 40 endpoints take 12.6 s at 8-way and 15.0 s at 40-way - five times the parallelism for **0.84x** the throughput, i.e. worse. A 598-device sweep projects to ~3m44s against a 30 s interval | snmpsim serves the whole fleet from one process behind a wildcard socket. Endpoints stay ONLINE because the timeouts are transient and recover, but the collector is continuously behind on SNMP. The operational lever is the poll interval (the `snmp-*` rows in `poll_profile`), not `max_concurrent`. Walking only the mapped columns rather than whole tables made a 65-port switch 3x faster (341 ms against 1.09 s) and is already applied |
| 34 | **RESOLVED by 32.** An ifXTable walk against a switch returned zero varbinds while the same agent served ifTable and served ifXTable on a 2-port server | the first ifXTable column after `ifName` is `ifHCInOctets`, which is exactly where the mis-tagged record was. After the fix the same switch returns 65 ports by name and 586 samples in 1.1 s. The server/switch asymmetry was walk ORDER, not device type |
